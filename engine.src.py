...
class Gen(object):   # Stores the logical structure of keywords and modules. A unique construct deserving a name.
    ...
    def check_memory(self):
        ...
        if self.parse_if('hse'):
            memory_required = ( (self.memory['projector_real'] + self.memory['projector_reciprocal'])*int(self.getkw('npar')) + 3*self.memory['wavefunction']*int(self.getkw('kpar')) )/1024/1024/1024 + int(self.getkw('nodes'))
        else:
            memory_required = ( (self.memory['projector_real'] + self.memory['projector_reciprocal'])*int(self.getkw('npar')) + self.memory['wavefunction']*int(self.getkw('kpar')) )/1024/1024/1024 + int(self.getkw('nodes'))*3/2
        memory_required *= multiplier
        memory_available = int(self.getkw('nodes')) * int(self.getkw('mempernode'))
