...
class Gen(object):   # Stores the logical structure of keywords and modules. A unique construct deserving a name.
    ...
    def evaluate(self,expression):                  # Parses a literal value or (funcname)
        if expression.startswith('(') and expression.endswith(')'): 
            ...
            if not func:
                print 'pre.parser error: Bad Grammar. Unable to find function {%s}' % re.sub('\(|\)','',expression)
            return func()
        else:   # literal
            return expression
	...
	# 执行require
        self.require = []
        self.moonphase = 1
	with open('compute.gen.csv') as conf:
	    lines = conf.read().splitlines()
            for line in [ [p.strip() for p in l.split(':')] for l in lines if not l.startswith('#') ]:
                if len(line) < 3:
                    print 'pre.parser error: Bar Grammar. Require line format wrong: {%s}' % line ; sys.exit(1)
        ...
        for line in self.require:
            if self.parse_if(line[0]):
                if line[2] == 'optional':
                    sys.stdout.write( 'pre.parser warning: conflict. Requirement is { %s : %s }. Explanation: { %s }.' % (line[0],line[1],line[3]) )
                else:
                    sys.stdout.write( 'pre.parser error: conflict. Requirement is { %s : %s }. Explanation: { %s }.' % (line[0],line[1],line[3]) )
	...
	for modname in set(self.mod.keys())-self.mod_legal_set:
            print 'pre.parser warning: illegal name. Mod {%s} is not in mod_legal_set and has been ignored. Mod_legal_set={%s}' %(modname,self.mod_legal_set)
	    del self.mod[modname]
	for kwname in set(self.kw.keys())-self.kw_legal_set:
	    print 'pre.parser warning: illegal name. Kw {%s} is not required and has been ignored.' % kwname
	    del self.kw[kwname]
        ...
        for name in self.kw:
            if len(self.kw[name]) != 1:
                print 'pre.parser error: non-unique output. Kw[%s]={%s} has not been restricted to 1 value.' %(name,self.kw[name]) ; sys.exit(1)
            else:
