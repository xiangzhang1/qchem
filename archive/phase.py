...
class Ket(object):
    ...
    def __init__(self, phase, cell, property_wanted, path=None, label=None) #
        self.phase, self.cell, self.property_wanted, self.path, self.label = phase, cell, property_wanted, path, label
        #--------------------------------------------------
        if not getattr(self, 'path', None):
            print 'Need path for this Ket \n'
            print '-------------------'
            pprint(self)
            print '-------------------'
            self.path = raw_input('> ')
        if not getattr(self, 'label', None):
            print 'Need label for this Ket \n'
            print '-------------------'
            pprint(self)
            print '-------------------'
            self.label = raw_input('> ')
        #--------------------------------------------------
        if self.path not in KETS:   #
            KETS[self.path] = self
    
    def moonphase(self):    # recursion
        if len(self.property_wanted.nodes) > 1:
            return self.property_wanted.moonphase()
        elif len(self.property_wanted.nodes) == 1:
            ...
            raise SystemError('len(%s.property_wanted.nodes)<1' %self.label)

    def compute(self):  # recursion or not
        if len(self.property_wanted.nodes) != 1:
            property_wanted_ = self.property_wanted.compute()
            subset = self.property_wanted.subset([ property_wanted ])
            path, label = self.property_wanted.node(property_wanted_)[1], self.property_wanted.node(property_wanted_)[2]    #
            if path not in KETS:    #
                KETS[path] = Ket(self.phase, self.cell, subset, path, label)    #
                KETS[path].compute()    #
            else:
                KETS[path].compute()    #
        else:
            self.gen = engine.Gen(' '.join(self.phase) + ' ' + self.property_wanted.nodes[0][0], self.cell)
    ...
    def consider(self, input_):
        return KETS[self.property_wanted.node(input_)[1]]   #
