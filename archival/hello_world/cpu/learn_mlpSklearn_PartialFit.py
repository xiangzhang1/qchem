...
for i in range(START+1, MAX+TEST):
    for _ in range(EPOCH):
        regr_1.partial_fit([X_train[i]], [Y_train[i]])
        regr_2.reset_states()
        regr_2.fit(X_train[:i], Y_train[:i])
    error_1 = np.linalg.norm(regr_1.predict([X_train[i+1]]) - [Y_train[i+1]])
    error_2 = np.linalg.norm(regr_2.predict([X_train[i+1]]) - [Y_train[i+1]])
    print i, error_1, error_2
