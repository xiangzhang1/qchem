...
with tf.variable_scope('ann_B', reuse=reuse):
    hidden_B1 = tf.layers.dense(X_B, n_hidden_B1, activation=tf.nn.relu)
    hidden_B2 = tf.layers.dense(hidden_B1, n_hidden_B2, activation=tf.nn.relu)
    y_B = tf.layers.dense(hidden_B2, 1)
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        for _ in range(data.shape[0] // batch_size):
            _X_batch, _y_batch, _y_A, _y_B, _y, _ = sess.run([X_batch, y_batch, y, y_A, y_B, training_op])
        if epoch % 100 == 0:
            print '-' * 50
            print 'Epoch %s \n\n X_batch %s \n\n y_batch %s \n\n X_A %s \n\n X_B %s \n\n y_A %s \n\n y_B %s \n\n y %s' %(epoch, _X_batch, _y_batch, _y_A, _y_B, _y)
            break
